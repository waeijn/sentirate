# start_all.ps1
# Run this from C:\sentirate

# 1. Redis
docker start rate-limiter-redis

# 2. Middleware (Backend API)
Start-Process powershell -ArgumentList '-NoExit', '-Command', 'cd C:\sentirate\dashboard\backend; C:\sentirate\.venv\Scripts\python.exe main.py'

# 3. Frontend (React UI)
Start-Process powershell -ArgumentList '-NoExit', '-Command', 'cd C:\sentirate\dashboard\frontend; npm run dev'
...
Write-Host "     cd C:\sentirate\experiments\traffic_generation\locust_scripts" -ForegroundColor Yellow
