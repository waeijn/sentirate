...
// ── Marker row with raw value display ─────────────────────────────────────────
...
function markerLabel(
  ...
  if (marker === "Rate (λ)") {
    ...
    if (value > 10) return `In bursty range (11–30 req/s)`;
    return `Within normal range (≤ 10 req/s)`;
  }
  if (marker === "Sigma (σ)") {
    if (sigma === null || sigma === undefined) return "Insufficient history";
    if (sigma < 0.1) return "Very low — machine-like timing precision";
    if (sigma < 0.5) return "Low — semi-regular timing";
    return "High — natural human timing variability";
  }
  if (marker === "Burst Frequency") {
    if (value > 5) return `Above suspicious threshold (> 5/min)`;
    if (value >= 3) return `In bursty range (3–5/min)`;
    return `Within normal range (0–2/min)`;
  }
  if (marker === "Persistence") {
    if (value > 60) return `Above suspicious threshold (> 60s)`;
    if (value >= 5) return `In bursty range (5–60s)`;
    return `Low — brief activity (< 5s)`;
  }
  return "";
}
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      ...
      >
        ...
        >
          {value === null ? "—" : displayValue.toFixed(decimals)}
          {value !== null && (
      ...
      {entry && (
        <>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px" }}>
            ...
            {/* Justification — from backend if available, fallback otherwise */}
            <h4
              style={{
                fontSize: 12,
                fontWeight: 600,
            ...
            >
              {entry.justification ||
                (entry.classification === "suspicious"
                  ? `Classified as Suspicious. One or more behavioral markers exceeded thresholds: rate λ=${entry.requestRate.toFixed(1)} req/s, σ=${entry.sigma?.toFixed(3) ?? "N/A"}s, burst freq=${entry.burstFreq}/min, persistence=${entry.persistence.toFixed(1)}s. Token bucket restricted to r=${entry.refillRate} tok/s, b=${entry.bucketCapacity} tokens.`
                  : entry.classification === "bursty"
                    ? `Classified as Bursty Legitimate. Rate λ=${entry.requestRate.toFixed(1)} req/s is in the elevated range (11–30 req/s). Pattern is consistent with batch operations or app sync. Token bucket expanded to r=${entry.refillRate} tok/s, b=${entry.bucketCapacity} tokens.`
                    : `Classified as Normal. Rate λ=${entry.requestRate.toFixed(1)} req/s is within the normal range (≤ 10 req/s). Default token bucket applied: r=${entry.refillRate} tok/s, b=${entry.bucketCapacity} tokens.`)}
            </p>
